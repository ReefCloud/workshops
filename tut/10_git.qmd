...
At this point, if we make a change to `file 1`, this file
will be in a _modified_ state until it is added to the _staging
area_. After committing, the current committed snapshot
(_snapshot 3_) is in sync with the _workspace_. 
...
of four important sections of the repository. In the figure below, the
left hand panel represents the contents of the root directory
(excluding the `.git` folder) - this is the _workspace_ and is
currently empty.
...
\tikzstyle{CODE} = [font={\fontspec[Scale=2]{Inconsolata}}] 
\tikzstyle{TREE} = [font={\fontspec[Scale=1.5]{Inconsolata}}] 
\tikzstyle{fileText} = [font={\fontspec[Scale=1.1]{Inconsolata}}] 
...
The add process also created a `index` file. This file
...
![](10_git_files/figure-html/drawGit5.png)
...
extension such as .aux. Hence, we can create a`.gitignore` to exclude
these. Lets start by modifying the `file2` and creating a new file
...
the <a href="#Branching">section on branching</a>.
...
file. The `git diff` allows us to explore differences between:
...
\commit{right = 1.5cm of commitshash3}{commitshash6}{color_commit}{commitshash6}{commitscomment6}
\draw [<-,line width=3pt,draw=black!60] (commitshash3.east) -- (commitshash6.west);
...
\commit{right = 1.5cm of commitshash3}{commitshash6}{color_inactive}{commitshash6}{commitscomment6}
\draw [<-,line width=3pt,draw=black!60] (commitshash3.east) -- (commitshash6.west);
\branch{below = 1cm of commitshash6}{Feature} 
\end{tikzpicture}
\end{preview}
...
\commit{right = 1.5cm of commitshash3}{commitshash8}{color_commit}{commitshash8}{commitscomment8}
\draw [<-,line width=3pt,draw=black!60] (commitshash3.east) -- (commitshash8.west);
    
\commit{above = 3cm of commitshash8}{commitshash6}{color_inactive}{commitshash6}{commitscomment6}
\draw [<-,line width=3pt,draw=black!60] (commitshash3.north east) -- (commitshash6.south west);
\branch{below = 1cm of commitshash6}{Feature} 

\master{below = 1cm of commitshash8} 
\HEAD{below = 0.1cm of master} 
\end{tikzpicture}
\end{preview}
\end{document}
...
\commit{right = 1.5cm of commitshash6}{commitshash10}{color_commit}{commitshash10}{commitscomment10}
\draw [<-,line width=3pt,draw=black!60] (commitshash6.east) -- (commitshash10.west);

\branch{below = 1cm of commitshash10}{Feature} 
\HEAD{below = 0.1cm of Feature} 
\end{tikzpicture}
\end{preview}
\end{document}
...
called a <b>merge</b> and is completed with the `git merge <branch>`
...
Hmmm. It appears that there is a conflict. If we explore the a `git
diff`, we will see that on the `master` and `Feature` _branches_ have
...
And compare that to the contents of `file1` in the `main` _commit_:
...
and the contents of `file1` in the `Feature` _commit_:
...
we can see that the changes made are inconsistent. We need to decide
which we want to use. Recall that the change made in `main` was to
...
as the `main` anyway.... Lets address this by rolling back `file1`
from the `main` _branch_ (of course we would normally use an editor to
edit those actual changes in the file back to the ancestral condition.
...
% new commit on main
\commit{right = 1.5cm of commitshash3}{commitshash8}{color_inactive}{commitshash8}{commitscomment8}
\draw [<-,line width=3pt,draw=black!60] (commitshash3.east) -- (commitshash8.west);

\commit{above = 3cm of commitshash8}{commitshash6}{color_inactive}{commitshash6}{commitscomment6}
\draw [<-,line width=3pt,draw=black!60] (commitshash3.north east) -- (commitshash6.south west);

\commit{right = 1.5cm of commitshash6}{commitshash10}{color_inactive}{commitshash10}{commitscomment10}
\draw [<-,line width=3pt,draw=black!60] (commitshash6.east) -- (commitshash10.west);

\commit{right = 5cm of commitshash8}{commitshash12}{color_commit}{commitshash12}{commitscomment12}
\draw [<-,line width=3pt,draw=black!60] (commitshash10.south east) -- (commitshash12.north west);
\draw [<-,line width=3pt,draw=black!60] (commitshash8.east) -- (commitshash12.west);

\master{below = 1cm of commitshash12} 
\branch{below = 1cm of commitshash10}{Feature} 
\HEAD{below = 0.1cm of master} 
...
If the bug fix is relatively small, it might be worth considering
<b>rebasing</b> into the main `main` _branch_ to prevent the history
becoming overly complex and messy to follow.
...
**Note**, this repository has an intentional conflict between the
`main` and `Feature` branches. This conflict occurs on line 3 of the
`file1` file. We saw this when we attempted to merge the `Feature`
branch into the `main` branch in the previous section. As with
merging, _rebasing_ can also be complicated by conflicts.
...
As with the `merge` in an earlier section, we will address the
conflict by reinstating the `main` version of `dir1/file2` and staging
it before continuing the `rebase`. When continuing the rebase, I will
preface the command with `GIT_EDITOR=true`. This will essentially
forgo poping up an editor for editing a commit message. Whilst not
necessary when running interactively, it is necessary when running git
within a script - such as what I am doing.
...
%New feature
\commit{right = 1.5cm of commitshash8}{commitshash16}{color_inactive}{commitshash16}{commitscomment16}
\draw [<-,line width=3pt,draw=black!60] (commitshash8.east) -- (commitshash16.west);

%Feature complete
\commit{right = 1.5cm of commitshash16}{commitshash17}{color_commit}{commitshash17}{commitscomment17}
\draw [<-,line width=3pt,draw=black!60] (commitshash16.east) -- (commitshash17.west);

\master{below = 1cm of commitshash8} 
\branch{below = 1cm of commitshash17}{Feature} 
\HEAD{below = 0.1cm of Feature} 
\end{tikzpicture}
\end{preview}
\end{document}
...
If we then checked out `main` the commit we made whilst in _detached
...
\begin{tikzpicture}
\rcommit{}{commitshash1}{white}{commitshash1}{commitscomment1}
\rmaster{below = 1cm of commitshash1} 
...
\node [anchor=north,TARGET] at ($(commitshash1.south) + (0,-2cm)$) {Local Repository};

\rcommit{below = 6cm of commitshash1}{rcommitshash1}{white}{commitshash1}{commitscomment1}
...
\rcommit{right = 1.5cm of rcommitshash2}{rcommitshash3}{color_commit}{commitshash3}{commitscomment3}
\draw [<-,line width=3pt,draw=black!60] (rcommitshash2) -- (rcommitshash3.west);
\rmaster{below = 1cm of rcommitshash3} 
\rHEAD{below = 0.1cm of rmaster} 

\node [anchor=north,TARGET] at ($(rcommitshash1.south) + (0,-2cm)$) (Local) {Remote Repository};
\draw [dashed] ($(Local.west |- rcommitshash1.north west) +(-0.5cm,0.5cm)$) rectangle ($(rHEAD.south east)
+(0.8cm,-0.5cm)$);
...
\draw [->,very thick,dashed] ($(commitshash3.east) +(0.5cm,0)$) to [out=10,in=0] ($(rcommitshash3.east) +(2cm,0)$);
...
superficially the opposite of _pushing_. Actually, it is two
actions: 
...
\commit{right = 1.5cm of commitshash3}{commitsrhash4}{color_commit}{commitsrhash4}{commitsrcomment4}
\draw [<-,line width=3pt,draw=black!60] (commitshash3) -- (commitsrhash4.west);
\master{below = 1cm of commitsrhash4} 
\HEAD{below = 0.1cm of master} 
\node [anchor=north west,TARGET] at ($(commitshash1.south) + (0,-2cm)$) {Local Repository (user 1)};

%user 2
\commit{below = 6cm of commitshash1}{commits2hash1}{color_inactive}{commitshash1}{commitscomment1}
...
\draw [<-,line width=3pt,draw=black!60] (commits2hash2) -- (commits2hash3.west);

\commit{right = 1.5cm of commits2hash3}{commits2rhash4}{color_commit}{commitsrhash4}{commitsrcomment4}
\draw [<-,line width=3pt,draw=black!60] (commits2hash3) -- (commits2rhash4.west);
\master{below = 1cm of commits2rhash4} 
\HEAD{below = 0.1cm of master} 
\node [anchor=north west,TARGET] at ($(commits2hash1.south) + (0,-2cm)$) {Local Repository (user 2)};

% remote
\rcommit{below right = 6cm of commitsrhash4}{rcommitshash1}{white}{commitshash1}{commitscomment1}
\rcommit{right = 2cm of rcommitshash1}{rcommitshash2}{color_inactive}{commitshash2}{commitscomment2}
\draw [<-,line width=3pt,draw=black!60] (rcommitshash1) -- (rcommitshash2.west);
\rcommit{right = 2cm of rcommitshash2}{rcommitshash3}{color_inactive}{commitshash3}{commitscomment3}
\draw [<-,line width=3pt,draw=black!60] (rcommitshash2) -- (rcommitshash3.west);
\rcommit{right = 2cm of rcommitshash3}{rcommits2rhash4}{color_commit}{commitsrhash4}{commitsrcomment4}
\draw [<-,line width=3pt,draw=black!60] (rcommitshash3) -- (rcommits2rhash4.west); 
\rmaster{below = 1cm of rcommits2rhash4} 
\rHEAD{below = 0.1cm of rmaster} 

\node [anchor=north west,TARGET] at ($(rcommitshash1.south) + (0,-2cm)$) (Remote) {Remote Repository};
\draw [dashed] ($(rcommitshash1.north west |- rcommitshash1.north west) +(-1cm,0.5cm)$) rectangle ($(rHEAD.south east)
+(0.8cm,-0.5cm)$);


\draw [<->,very thick,dashed] ($(commitsrhash4.east) +(0.5cm,0)$) to [out=10,in=200] ($(rcommitshash1.east) +(-2cm,0)$);
\draw [<->,very thick,dashed] ($(commits2rhash4.east) +(0.5cm,0)$) to [out=10,in=200] ($(rcommitshash1.east) +(-2cm,0)$);
...
rcommits = system("cd ~/tmp/RemoteRepo1; git log --oneline | awk '{ print $1,\"\\t\",substr($0, 9) }'", intern=TRUE)
rcommits = read.table(textConnection(rcommits), header=FALSE, strip.white=TRUE, sep='\t')
rcommits = rcommits[rev(1:nrow(rcommits)),]
colnames(rcommits) <- c('hash','comment')
rcommits$comment <- paste0(strtrim(gsub('.*:','',rcommits$comment),10),'..')	
for (i in 1:dim(rcommits)[1]) {
  system(paste0("sed -i 's/commitsrhash",i,"\\([\\.\\})]\\)/", rcommits$hash[i], "\\1/g' resources/Fig10c.tikz")) 
    system(paste0("sed -i 's/commitsrcomment",i,"\\([\\.\\})]\\)/", rcommits$comment[i], "\\1/g' resources/Fig10c.tikz")) 
}
