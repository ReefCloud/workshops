...
\beta_2(dummy_2)_{ij} + .... + \varepsilon_{ij}$$ where $i$ represents
the treatment levels (from 1 to $p$) and $j$ represents the set of
replicates (from 1 to $n$) within the $i^{th}$ group. Hence, $y_{ij}$
represents the $j^{th}$ observation of the response variable within
the $i^{th}$ group and $(dummy_1)_{ij}$ represents the dummy code for
the $j^{th}$ replicate within the $i^{th}$ group of the first dummy
...
parameters from three populations, see <a
href="#linearmodelsEstimation">Estimation</a>).


An effects model for a factor with $p$ groups, will have $p+1$
parameters (the overall mean $\mu$ plus the $p$ $\alpha$ parameters),
and thus the linear effects model is considered to be
**over-parameterized**. Given that $\alpha_i=\mu_i - \mu$, it is only
...
- **means parameterization** - removing one of the parameters from the
  ...
  treatment effects ($\alpha_i$) parameters - a procedure rarely used
  ...
  $$y_i = \beta_p+\varepsilon_i, \hspace{2cm}\text{where}~p= \text{number of levels of the factor}$$
  $$\begin{pmatrix}
...
- **effects parameterization** - generating a new set ($p-1$) of
  effects parameters ($\beta^*_{q}$, where $q$ represents the set of
  orthogonal parameters from 2 to $p$) each of which represent a
  linear combination of groups rather than a single group effect. That
  is, each $\beta^*$ can include varying contributions from any
  number of the groups and are not restricted to a single contrast of
  ($=\beta_p-\beta$). For example, one of the parameters might represent
  the difference in means between two groups or the difference in
  means between one group and the average of two other groups. $$y_i =
  \beta_0+\beta_p+\varepsilon_i, \hspace{2cm}\text{where}~p= 2:p$$ In the
  effects parameterization, $\mu$ typically represents the mean of one
  of the groups (a reference group) and each of the $\beta$ effects
  represent the difference between subsequent group means and the
  reference group.
...
A number of `pre-fabricated', contrast matrices exist (to yield
...
applied components of this tutorial will be motivated by two
fabricated data sets - one that simulates samples drawn from a
Gaussian (normal) distribution and one that simulates samples drawn
from a Poisson distribution.

The advantage of simulated data over real data is that with simulated
data, we know the 'truth' and can therefore gauge the accuracy of
estimates.

::: {.panel-tabset}

## Motivating example 1 (Paussian data)
...
## Motivating example 2 (Poisson data)
...
one unit increase in the predictor the response increaes by 50% (on
...
beta <- log(beta)
n <- 10
dat2 <- data.frame(x=seq(from = 1, to = 10, len = n)) |>
...
## Motivating example 3 (categorical predictor)
...
might not be appropriate for the motivating example 2 data. To see how
...
#| fig-height: 10
#| fig-width: 8
dat2.lm |> performance::check_model()
dat2.lm |> performance::check_outliers()
```
...
#| echo: true
#| cache: false
dat3a.lm <- lm(y ~ x, data = dat3)
dat3a.lm
```

**Interpretation**

- the mean of the first group is `r coef(dat3a.lm)[1]`
- the difference between the means of the first and second group (i.e.
  the effect of the second group over the first) is `r coef(dat3a.lm)[2]`
- the difference between the means of the first and third group (i.e.
  the effect of the third group over the first) is `r coef(dat3a.lm)[3]`
...
#| echo: true
#| cache: false
dat3b.lm <- lm(y ~ -1 + x, data = dat3)
dat3b.lm
```

**Interpretation**

- the mean of the first group is `r coef(dat3b.lm)[1]`
- the mean of the second group is `r coef(dat3b.lm)[2]`
- the mean of the third group is `r coef(dat3b.lm)[3]`
...
- the mean of the groups is `r coef(dat3c.lm)[1]`
...
- the mean of the groups is `r coef(dat3d.lm)[1]`
- the difference between the mean of the first group and the mean
  of the first and second groups is `r coef(dat3d.lm)[2]`
- the difference between the mean of the second group and the mean
  of the second and third groups is `r coef(dat3d.lm)[2]`
...
- the mean of the groups is `r coef(dat3e.lm)[1]`
- $y$ declines at a rate of `r coef(dat3e.lm)[2]` per level of the
  group ($x$). There is a negative relationship.
- the rate of quadratic curvature is `r coef(dat3e.lm)[3]`. Since this
...
library(gmodels)
contrasts(dat3$x) <- make.contrasts(rbind(c(1, -0.5, -0.5)))
contrasts(dat3$x)
model.matrix(~x, data =  dat3)
```
...
- the mean of the groups is `r coef(dat3f.lm)[1]`
- the difference between the first group and the average of the second
  and third groups is `r coef(dat3f.lm)[2]`
...
This upcoming section contains a lot of detail that some might find
excessive and unnecessary if we only need to know how to fit a
...
statistical routines, the follwing summary might suffice for those
seeking a quicker overview.
...
#| echo: true
#| cache: false
dat2.glm <- glmmTMB(y~x, data = dat2, family = poisson(link = "log"))
dat2.glm
```
...
#| fig-height: 10
#| fig-width: 8
dat2.glm |> performance::check_model()
dat2.glm |> performance::check_outliers()
```
...
After `r optim.IRLS(cbind(rep(1,length(dat2$y))), dat2$y,
family=poisson)$Iter` iterations, the algorithm converged on an
estimate of `r optim.IRLS(cbind(rep(1,length(dat2$y))), dat2$y,
family=poisson)$B.optim`, which on the natural scale would be `r
exp(optim.IRLS(cbind(rep(1,length(dat2$y))), dat2$y,
...
glm(y ~ 1, data=dat2, family = poisson(link = "log"))
```
...
glm(y ~ x, data=dat2, family = poisson(link = "log"))
```
